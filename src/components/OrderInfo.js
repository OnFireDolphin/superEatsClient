...
function OrderInfo(props) {
    ...
    useEffect(() => {
        axios
            .get('https://supereatsserver.azurewebsites.net/orders/byId/'.concat(props.match.params.id))
            .then((orderres) => {
                setOrder(orderres.data);

                axios
                    .get('https://supereatsserver.azurewebsites.net/inventory/')
                    .then((invres) => {
                        var currsuccItems = [];
                        var currUnsuccItems = [];
                        var currChanges = [];
                                ...
                                if (quickScore(invres.data[j].product, orderres.data.items[i].product) > 0.8) {
                                    if (orderres.data.items[i].quantity <= invres.data[j].stock) {
                                        ...
                                        currChanges.push({
                                            ...
                                            stock: invres.data[j].stock - orderres.data.items[i].quantity,
    ...
    return (
        <div className="container">
            <div className="upMargin">
                <h1 className="header-with-desc" style={{ textTransform: 'capitalize' }}>
                    Order: {order.name}
                </h1>
                    ...
                    <tbody style={{ textTransform: 'capitalize' }}>
                        ...
                        {order.items == undefined
                            ...
                            : order.items.map((item) => {
                                  const success = succItems.some((succItem) => succItem.product == item.product)
                                      ? true
                                      : false;
                                  return (
                                      <tr>
                                          <td>
                                              <span
                                                  className={success ? 'text-success' : 'text-danger font-weight-bold'}
                                              >
                                                  {item.product}
                                              </span>
                                          </td>
                ...
                <button
                    ...
                    onClick={() => {
                        axios.put(`https://supereatsserver.azurewebsites.net/orders/byId/${order._id}`, {
                            ...order,
                        ...
                        for (var i = 0; i < changes.length; i++) {
                            axios
                                .put(`https://supereatsserver.azurewebsites.net/inventory/byId/${changes[i].id}`, {
